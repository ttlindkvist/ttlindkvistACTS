...
class CustomVertexSeeder {
 ...
 public:
  ...
  struct Config {
    ...
    Config(const Acts::ImpactPointEstimator<InputTrack_t, Propagator_t>& ipEst)
        : ipEstimator(ipEst) {}

    // ImpactPointEstimator
    Acts::ImpactPointEstimator<InputTrack_t, Propagator_t> ipEstimator;

    // FsmwMode1dFinder
    Acts::FsmwMode1dFinder mode1dFinder;
    ...
    bool usePt = false;
    // minimum pt
    double minPt = 0.4 * Acts::UnitConstants::GeV;
  ...
  template <
      ...
      std::enable_if_t<std::is_same<T, Acts::BoundTrackParameters>::value, int> = 0>

  CustomVertexSeeder(const Config& cfg,
                    std::unique_ptr<const Acts::Logger> logger =
                        Acts::getDefaultLogger("CustomVertexSeeder", Acts::Logging::INFO))
      : m_cfg(std::move(cfg)),
  ...
  CustomVertexSeeder(const Config& cfg,
                    std::function<Acts::BoundTrackParameters(InputTrack_t)> func,
                    std::unique_ptr<const Acts::Logger> logger =
                        Acts::getDefaultLogger("CustomVertexSeeder", Acts::Logging::INFO))
  ...
  Acts::Result<std::vector<Acts::Vertex<InputTrack_t>>> find(
      const std::vector<const InputTrack_t*>& trackVector,
      const Acts::VertexingOptions<InputTrack_t>& vertexingOptions,
      State& state) const;
 ...
 private:
  ...
  std::function<Acts::BoundTrackParameters(InputTrack_t)> m_extractParameters;
...
#include "CustomVertexSeeder.ipp"
