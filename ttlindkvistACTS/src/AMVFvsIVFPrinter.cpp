...
ttlindkvist::AMVFvsIVFPrinter::AMVFvsIVFPrinter(
    const Config& config, Acts::Logging::Level level)
    : ActsExamples::BareAlgorithm("AMVFvsIVFPrinter", level),
      m_cfg(config) {
  if(m_cfg.outputDir.empty()) {
    throw std::invalid_argument("Missing output directory");
  }
  if(m_cfg.generatedTrackParameters.empty()) {
    throw std::invalid_argument("Missing input track parameters");
  }
}

ActsExamples::ProcessCode ttlindkvist::AMVFvsIVFPrinter::execute(
    const ActsExamples::AlgorithmContext& ctx) const {
    
  const auto& inputTracks = ctx.eventStore.get<std::vector<Acts::BoundTrackParameters>>(m_cfg.generatedTrackParameters);
  
  if(/*ctx.eventNumber > -1*/ true){
    //Open event file
    std::ofstream outputFile(boost::str(boost::format("%1%/event%2%_pythia8_cov.txt") % m_cfg.outputDir % ctx.eventNumber));
    outputFile << inputTracks.size() << std::endl; 
    
    //Print covariance matrix for some tracks
    int trackCounter = 0;
  ...
  if(!m_cfg.iterativeRecoVtxParameters.empty()){
    ...
    outputFile << inputTracks.size() << " " << IVFVtxs.size() << std::endl; 
  ...
  if(!m_cfg.AMVFRecoVtxParameters.empty()){
    ...
    outputFile << inputTracks.size() << " " << AMVFVtxs.size() << std::endl; 
