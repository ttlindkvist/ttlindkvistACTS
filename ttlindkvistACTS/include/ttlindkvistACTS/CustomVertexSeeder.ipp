...
auto ttlindkvist::CustomVertexSeeder<vfitter_t>::find(
    const std::vector<const InputTrack_t*>& trackVector,
    const Acts::VertexingOptions<InputTrack_t>& vertexingOptions,
    State& /*state*/) const -> Acts::Result<std::vector<Acts::Vertex<InputTrack_t>>> {
  ...
  for (const auto& iTrk : trackVector) {
    ...
    const Acts::BoundTrackParameters& params = m_extractParameters(*iTrk);

    std::pair<double, double> z0AndWeight;
    Acts::ImpactParametersAndSigma ipas;
    if (useConstraint &&
    ...
    if (ipas.sigmad0 > 0) {
      ...
      z0AndWeight.first = params.position(vertexingOptions.geoContext)[Acts::eZ];
      z0AndWeight.second = 1.;
    }
    ...
    if (!m_cfg.disableAllWeights && (m_cfg.usePt || m_cfg.useLogPt)) {
      double Pt =
          std::abs(1. / params.parameters()[Acts::BoundIndices::eBoundQOverP]) *
          std::sin(params.parameters()[Acts::BoundIndices::eBoundTheta]);
      if (m_cfg.usePt) {
  ...
  Acts::Vector4 output(vertexingOptions.vertexConstraint.position().x(),
                 vertexingOptions.vertexConstraint.position().y(), ZResult,
                 vertexingOptions.vertexConstraint.time());
  Acts::Vertex<InputTrack_t> vtxResult = Acts::Vertex<InputTrack_t>(output);
